...
// Password validation
const password: ValidationChain = check("password")
  .isLength({ min: 6, max: 15 })
  .withMessage("Password has to be between 6 and 15 characters.");

// Email validation
const email: ValidationChain = check("email")
  .isEmail()
...
const emailExistsCheck: ValidationChain = check("email").custom(
  async (value) => {
...
// Login validation
const loginFieldCheck: ValidationChain = check("email").custom(
  async (value, { req }) => {
